import type {A2UIMessage} from '../../protocol/a2ui.js';
...
export function createRestaurantListMessages(
  title: string = 'Top 5 Chinese Restaurants in New York',
  items: RestaurantItem[] = sampleRestaurants
): A2UIMessage[] {
  return [
    {
      beginRendering: {
    ...
    },
    {
      dataModelUpdate: {
        ...
        contents: [
          ...
          {
            key: 'items',
            valueMap: items.map((restaurant, index) => ({
              key: `item${index + 1}`,
              valueMap: [
